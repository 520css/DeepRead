...
import { Book, Box, StickyNote, FileText } from 'lucide-react';
...
export default function PaperView() {
    ...
    const handleCitationClick = useCallback((key: string, messageIndex: number, searchText?: string) => {
        setActiveCitationKey(key);
        setActiveCitationMessageIndex(messageIndex);

        // Page reference like (P.2) or [P.2] → jump directly to that page
        const pageMatch = key.match(/^P\.(\d+)/i);
        if (pageMatch) {
            setJumpToPage(parseInt(pageMatch[1]));
            // If citation has English snippet, also search for it in PDF
            if (searchText) {
                setTimeout(() => setExplicitSearchTerm(searchText), 500);
            }
    ...
    const handleAnnotateRequest = useCallback((highlightId: string) => {
