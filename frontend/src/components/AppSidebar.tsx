...
export function AppSidebar() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const [papersResponse, conversationsResponse, projectsResponse, agentResponse] = await Promise.all([
                    fetchFromApi("/api/paper/active"),
                    fetchFromApi("/api/conversation/everything"),
                    fetchFromApi("/api/projects"),
                    fetchFromApi("/api/agent/conversations").catch(() => ({ conversations: [] })),
                ]);
                ...
                setEverythingConversations(conversationsResponse?.conversations || conversationsResponse || []);
                setProjects(projectsResponse?.projects || projectsResponse || []);
                setAgentConversations(agentResponse?.conversations || []);
            } catch (error) {
                                ...
                                if (item.title === "Agent") {
                                    return (
                                        <CollapsibleSidebarMenu
                                            ...
                                            getItemUrl={(convo) => `/agent?conv_id=${convo.id}`}
                                            viewAllUrl="/agent"
                                            viewAllText="New agent chat"
                                            defaultOpen={false}
                                            maxItems={5}
                                        />
                                    )
                    ...
                    <SidebarMenuItem>
                        <SidebarMenuButton asChild>
                            <div className="w-full flex items-center gap-2 px-3 py-2 rounded-md">
                                <UserIcon size={16} />
                                <span className="font-medium text-muted-foreground">Local Mode</span>
                            </div>
                        </SidebarMenuButton>
                    </SidebarMenuItem>
                )}
