...
export function HighlightContainer({ onHighlightClick }: HighlightContainerProps) {
	...
	const isAnnotation = !!(highlight as any).hasAnnotation;
	...
	if (isTextHighlight) {
		return (
			<div
				...
				data-annotation={isAnnotation ? "true" : undefined}
				onClick={handleClick}
			...
			>
				<TextHighlight
					...
					highlight={highlight}
					style={{
						background: isAnnotation ? "transparent" : highlightColor,
					}}
					className={isAnnotation ? "annotation-underline" : undefined}
				/>
			</div>
		);
	}
