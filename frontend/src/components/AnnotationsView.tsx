													...
													<div className="flex flex-col gap-2" onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
														<textarea
															...
															onKeyDown={e => {
																if (e.key === 'Enter' && !e.shiftKey) {
																	e.preventDefault();
																	updateAnnotation?.(editingAnnotationId!, editDraft.trim());
														...
														<div className="flex gap-1">
															...
															<Button size="sm" onClick={() => { updateAnnotation?.(editingAnnotationId!, editDraft.trim()); setEditingAnnotationId(null); }} disabled={!editDraft.trim()}>Save</Button>
														</div>
													</div>
												) : (
													<div
														onClick={e => { e.stopPropagation(); setEditingAnnotationId(annotation.id); setEditDraft(annotation.content || ''); }}
														className="cursor-pointer hover:bg-muted/30 rounded -mx-1 px-1 transition-colors"
														title="Click to edit"
													>
														<CollapsibleNoteText
