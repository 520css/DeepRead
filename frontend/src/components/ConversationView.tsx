...
import { Loader, ArrowUp, Recycle, X, ChevronDown, ChevronUp, BookOpen, Pencil, ChevronLeft, ChevronRight } from "lucide-react";
...
interface ConversationViewProps {
	...
	// Tree conversation (optional — enables edit + version nav)
	conversationId?: string | null;
	conversationParentId?: string | null;
	conversationChildren?: Array<{ id: string; title: string; branchPointIndex: number }>;
	...
	const memoizedMessages = messages.map((msg, index) => (
		...
		>
			...
			>
				...
				{/* Edit + version nav for user messages (tree feature) */}
				{msg.role === "user" && onEditMessage && (
					<div className="flex items-center gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
						{conversationParentId && branchPointIndex > 0 && index === branchPointIndex && (
							<button
								className="h-5 w-5 p-0 text-muted-foreground hover:text-foreground"
								title="回到父对话"
								onClick={() => onSwitchConversation?.(conversationParentId!)}
							>
								<ChevronLeft className="size-3" />
							</button>
						)}
						{conversationChildren && conversationChildren.length > 0 && conversationChildren[0].branchPointIndex > 0 && index === conversationChildren[0].branchPointIndex && (
							<button
								className="h-5 w-5 p-0 text-muted-foreground hover:text-foreground"
								title="到子对话"
								onClick={() => onSwitchConversation?.(conversationChildren[0].id)}
							>
								<ChevronRight className="size-3" />
							</button>
						)}
						<button
							className="h-5 px-1 text-xs text-muted-foreground hover:text-foreground flex items-center gap-0.5"
							onClick={(e) => {
					...
					<div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
						...
						>
							...
							{msg.role === "assistant" && (
								...
								{msg.guardResult && (
									<GuardSummaryBar guardInfo={parseGuardResult(msg.guardResult)} />
								)}
								</>
							)}
						</div>
					...
					<form onSubmit={handleNewSubmit} className="w-full transition-all duration-300 ease-in-out" ref={chatInputFormRef}>
						<div className="relative w-full transition-all duration-300 ease-in-out">
							<Textarea
								...
								disabled={isStreaming || chatCreditLimitReached || !isOwner}
								onKeyDown={(e) => {
